// ignore_for_file: non_constant_identifier_names, constant_identifier_names, curly_braces_in_flow_control_structures, camel_case_extensions, use_build_context_synchronously
...
class GState {
  ...
  static final connectionStatus =
      SharedValue(value: WSAPeriodicConnector.alertStatus);
  static final ipAddress = PersistableValue<String>(
      value: "127.0.0.1",
      loader: (o) => o.ipAddress.asIpv4,
      setter: (o, e) => o.ipAddress = e.ipv4AsInt ?? IntUtils.LOCALHOST);
  static final androidPort = PersistableValue<int>(
      value: 58526, loader: (o) => o.port, setter: (o, e) => o.port = e);
  static final androidPortPending =
      SharedValue(value: androidPort.$.toString());
  // Interface options
  static final locale = PersistableValue<NamedLocale>(
      value: LocaleUtils.SYSTEM_LOCALE,
      loader: (o) => LocaleUtils.fromLCIDOrDefault(o.locale),
      setter: (o, e) => o.locale = e.lcid);
  // Theme options
  static final theme = PersistableValue<Options_Theme>(
      value: Options_Theme.SYSTEM,
      loader: (o) => o.theme,
      setter: (o, e) => o.theme = e);
  static final iconShape = PersistableValue<Options_IconShape>(
      value: Options_IconShape.SQUIRCLE,
      loader: (o) => o.iconShape,
      setter: (o, e) => o.iconShape = e);
  static final legacyIcons = PersistableValue<bool>(
      value: false,
  ...
  static final mica = PersistableValue<Options_Mica>(
      ...
      loader: (o) => o.mica,
      setter: (o, e) => o.mica = e);
  static final autostartWSA = PersistableValue<bool>(
      value: false,
      loader: (o) => o.autostart,
      setter: (o, e) => o.autostart = e);
  static final installTimeout = PersistableValue<int>(
      value: 30, loader: (o) => o.timeout, setter: (o, e) => o.timeout = e);
  // APK Info
  ...
  static final apkInstallState =
      SharedValue<InstallState>(value: InstallState.PROMPT);
  ...
  static final installCallback = SharedValue<
      Function(String ipAddress, int port, AppLocalizations lang, int timeout,
          [bool downgrade])?>(value: null);
  // Installation info
...
extension Options_IconShape_Ext on Options_IconShape? {
  double get radius {
    switch (this) {
      case Options_IconShape.SQUIRCLE:
        return 0.6;
      case Options_IconShape.CIRCLE:
        return 1;
      case Options_IconShape.ROUNDED_SQUARE:
        return 0.35;
      default:
        return 0.6;
    }
  ...
  String description(AppLocalizations lang) {
    switch (this) {
      case Options_IconShape.SQUIRCLE:
        return lang.theme_icon_adaptive_squircle;
      case Options_IconShape.CIRCLE:
        return lang.theme_icon_adaptive_circle;
      case Options_IconShape.ROUNDED_SQUARE:
        return lang.theme_icon_adaptive_rounded_square;
      default:
        return UNLOCALIZED_OPTION;
    }
  }
...
extension Options_Theme_Mode on Options_Theme? {
  ThemeMode get mode {
    switch (this) {
      case Options_Theme.SYSTEM:
        return ThemeMode.system;
      case Options_Theme.LIGHT:
        return ThemeMode.light;
      case Options_Theme.DARK:
  ...
  String description(AppLocalizations lang) {
    switch (this) {
      case Options_Theme.SYSTEM:
        return lang.settings_option_generic_system;
      case Options_Theme.LIGHT:
        return lang.theme_mode_light;
      case Options_Theme.DARK:
        return lang.theme_mode_dark;
      default:
        return UNLOCALIZED_OPTION;
    }
  }
...
class AppOptions {
  ...
  static Future<Options> _optionsFuture = () async {
    ...
    final oldDirectory = Directory(
        "${Env.USER_PROFILE}${RegExp(r'.*[/\\]$').hasMatch(Env.USER_PROFILE) ? '' : r'\'}.wsamanager\\");
    // TODO options file migration; remove eventually
    final oldOtionsFile = File("${oldDirectory.path}\\options.bin");
    final directory = Directory(
        "${Env.USER_PROFILE}${RegExp(r'.*[/\\]$').hasMatch(Env.USER_PROFILE) ? '' : r'\'}.wsa-pacman\\")
      ..createSync();
    // TODO options file migration; remove eventually
    if (oldDirectory.existsSync()) {
      if (oldOtionsFile.existsSync()) {
        if (!File("${directory.path}\\options.bin").existsSync())
          oldOtionsFile.copySync("${directory.path}\\options.bin");
        oldOtionsFile.deleteSync();
      }
      oldDirectory.delete(recursive: false);
    }
    _optionsFile = File("${directory.path}\\options.bin")
      ..createSync()
      ..openSync();
    ...
    } on InvalidProtocolBufferException catch (_) {
      return Options();
    }
  }();
  ...
  static void _checkSettingsFileChange() async {
    ...
    await AppOptions._fileLock.synchronized(() async {
      final newLastModified = _optionsFile!.lastModifiedAccurate() ??
          await _optionsFile!.lastModified();
    ...
    if (shouldUpdate) {
      ...
      _optionsFuture = () async {
        ...
        try {
          options =
              (_options = Options.fromBuffer(_optionsFile!.readAsBytesSync()));
        } on InvalidProtocolBufferException catch (_) {
  ...
  static Future withOptions(Function(Options options) setter) {
    return _protoLock.synchronized(() async {
...
class PersistableValue<T> extends SharedValue<T> {
  ...
  static void reinitializeAll() => AppOptions.withOptions((options) {
        for (final reinitializer in _reinitializers) reinitializer(options);
      });

  PersistableValue(
      {super.key,
      required super.value,
      required T Function(Options options) loader,
      required Function(Options options, T value) setter,
      super.autosave})
      : _setter = setter {
    reinitializer(Options options) {
      super.setIfChanged(loader(options));
      _initializer = null;
    }
  ...
  static Future persistOptions() {
    return AppOptions._fileLock.synchronized(() async {
      File optionsFile;
      if (AppOptions._optionsFile != null)
      ...
      await AppOptions.withOptions(
          (options) => optionBytes = options.writeToBuffer());
  ...
  void _saveOptions() =>
      AppOptions.withOptions((options) => _setter(options, $));

  void doWhenReady(void Function(T value) callback, [BuildContext? context]) {
    final initializer = _initializer;
    if (initializer != null)
      initializer.then((_) => callback(context == null ? $ : of(context)));
    else
      callback(context == null ? $ : of(context));
  }
