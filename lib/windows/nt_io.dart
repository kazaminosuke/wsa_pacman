// ignore_for_file: camel_case_types, constant_identifier_names, non_constant_identifier_names, curly_braces_in_flow_control_structures, unused_local_variable
...
base class REPARSE_MOUNTPOINT_DATA_BUFFER extends Struct {
  ...
  @WORD()
  external int reparseTargetLength;
  @WORD()
  external int reparseTargetMaximumLength;
  @WORD()
  external int reserved1;
  @Array(_ANYSYZE_ARRAY)
  external Array<WCHAR> reparseTarget;
}
...
base class _TOKEN_PRIVILEGES extends Struct {
  ...
  @Array(_ANYSYZE_ARRAY)
  external Array<_LUID_AND_ATTRIBUTES> privileges;
}
...
final _NtCreateSymbolicLinkObject = ntdll.lookupFunction<
    Uint32 Function(
        Pointer<IntPtr> linkHandle,
        Uint32 desiredAccess,
        Pointer<OBJECT_ATTRIBUTES> objectAttributes,
        Pointer<UNICODE_STRING> linkTarget),
    int Function(
        Pointer<IntPtr> linkHandle,
        int desiredAccess,
        Pointer<OBJECT_ATTRIBUTES> objectAttributes,
        Pointer<UNICODE_STRING> linkTarget)>('NtCreateSymbolicLinkObject');
final _NtStatusToDosError = ntdll.lookupFunction<Uint32 Function(Int32 status),
    int Function(int status)>('RtlNtStatusToDosError');
final _CreateDirectoryObject = ntdll.lookupFunction<
    Uint32 Function(
        Pointer<IntPtr> lpHandle,
        Uint32 desiredAccess,
        Pointer<OBJECT_ATTRIBUTES> obj_attr,
    ...
    int Function(
        ...
        int desiredAccess,
        Pointer<OBJECT_ATTRIBUTES> obj_attr,
        int shadowDirectoryHandle,
        int flags)>('NtCreateDirectoryObjectEx');
...
final _NtOpenSection = ntdll.lookupFunction<
    Uint32 Function(Pointer<IntPtr> lpHandle, Uint32 desiredAccess,
        Pointer<OBJECT_ATTRIBUTES> obj_attr),
    int Function(Pointer<IntPtr> lpHandle, int desiredAccess,
        Pointer<OBJECT_ATTRIBUTES> obj_attr)>('NtOpenSection');
...
final _LookupPrivilegeValue = advapi32.lookupFunction<
    ...
    int Function(LPWSTR lpSystemName, LPWSTR lpName,
        Pointer<LUID> lpLuid)>('LookupPrivilegeValueW');
final _AdjustTokenPrivileges = advapi32.lookupFunction<
    Uint32 Function(
        HANDLE tokenHandle,
        BOOL disableAllPrivileges,
        Pointer<_TOKEN_PRIVILEGES> newState,
        DWORD bufferLength,
        Pointer<_TOKEN_PRIVILEGES> previousState,
    ...
    int Function(
        ...
        int bufferLength,
        Pointer<_TOKEN_PRIVILEGES> previousState,
        Pointer<DWORD> returnLength)>('AdjustTokenPrivileges');
...
extension on String {
  ...
  Pointer<REPARSE_MOUNTPOINT_DATA_BUFFER> toReparseMountpoint() {
    ...
    final lpReparseBuffer = calloc.allocate<REPARSE_MOUNTPOINT_DATA_BUFFER>(
        sizeOf<REPARSE_MOUNTPOINT_DATA_BUFFER>() +
            (rTargetUnits.length + 1) * sizeOf<WCHAR>());
    final reparseBuffer = lpReparseBuffer.ref;

    final Pointer<Uint16> lpTarget = (lpReparseBuffer.cast<BYTE>() +
            (2 * sizeOf<DWORD>() + 4 * sizeOf<WORD>()))
        .cast<Uint16>();
    final Uint16List nativeString =
        lpTarget.asTypedList(rTargetUnits.length + 1);
    ...
    reparseBuffer.reparseTargetMaximumLength =
        (rTargetUnits.length + 1) * sizeOf<WCHAR>();
...
class NtIO {
  ...
  static final NT_TEMP_DIR_NAME = Directory(WinPath.tempSubdir)
      .createTempSync("WSA-PacMan-DOS@$pid@")
      .basename;
  ...
  static final int? _NT_JUNCTION_HANDLE = (_DOS_DIRECTORY != null)
      ? createJunction(
          _DOS_DIRECTORY!, "${WinPath.tempSubdir}\\$NT_TEMP_DIR_NAME", true)
      : null;
  ...
  static String? createTempShortcut(String target, String shortcutName) {
    ...
    if (directory != null) {
      NtIO.createNativeSymlink(
          0, "\\??\\$target", "$_DOS_DIRECTORY\\$shortcutName");
      _NT_JUNCTION_HANDLE;
  ...
  static void deleteNtTempDirJunction() => (_NT_JUNCTION_HANDLE != null)
      ? CloseHandle(_NT_JUNCTION_HANDLE!) != 0
      : false;

  /// Opens a directory, returns its handle
  static int? openDirectory(String path, bool bReadWrite,
    ...
    try {
      ...
      _LookupPrivilegeValue(
          nullptr,
          bReadWrite ? _SE_RESTORE_NAME : _SE_BACKUP_NAME,
          (lpTp.cast<DWORD>() + 1).cast<LUID>());
      tp.privilegeCount = 1;
      tp.privileges[0].attributes = _SE_PRIVILEGE_ENABLED;
      final hToken = lpToken.value;
      _AdjustTokenPrivileges(
          hToken, FALSE, lpTp, sizeOf<_TOKEN_PRIVILEGES>(), nullptr, nullptr);
      CloseHandle(hToken);
  ...
  static int? createJunction(String targetDir, String symlinkDir,
    ...
    try {
      int result = dirHandle != null
          ? DeviceIoControl(
              dirHandle,
              FSCTL_SET_REPARSE_POINT,
              lpReparseBuffer,
              lpReparseBuffer.ref.reparseDataLength +
                  _REPARSE_MOUNTPOINT_HEADER_SIZE,
              nullptr,
              0,
              lpBytesReturned,
              nullptr)
          : 0;
      if (result == 0)
        log("\x1B[91mJunction point creation failed: ${getMessageDOS(GetLastError())}");
      if (result == 0 && dirHandle != null && deleteOnClose)
  ...
  static Pointer<OBJECT_ATTRIBUTES> _InitializeObjectAttributes(
  ...
  static String getMessageNt(int code) =>
      getMessageDOS(_NtStatusToDosError(code));
  ...
  static int? createNativeDirectory(String nativePath) {
    ...
    final objAttrs = _InitializeObjectAttributes(
        objectName, OBJ_CASE_INSENSITIVE, 0, nullptr);
    final lpHandle = calloc<IntPtr>();
    try {
      int result = _CreateDirectoryObject(
          lpHandle, DIRECTORY_ALL_ACCESS, objAttrs, 0, 0);
      if (result != 0)
        log("\x1B[91mNative directory cration failed: ${getMessageNt(result)}",
            level: 1000);
      else
        return lpHandle.value;
    } finally {
      objectName.free();
      free(objAttrs);
      free(lpHandle);
  ...
  static bool openSection(String nativePath, [bool logErrors = true]) {
    ...
    try {
      ...
      if (result != 0) {
        if (logErrors)
          log("\x1B[91mFailed to open section in object manager: ${getMessageNt(result)}",
              level: 1000);
        return false;
      } else {
  ...
  static int? createNativeSymlink(
      int rootDirHandle, String target, String symlink) {
    final lpTarget = target.toUnicodeString();
    final lpSymlink = symlink.toUnicodeString();
    final attributes = _InitializeObjectAttributes(
        lpSymlink, OBJ_CASE_INSENSITIVE, rootDirHandle, nullptr);
    final lpHandle = calloc<IntPtr>();
    try {
      int result = _NtCreateSymbolicLinkObject(
          lpHandle, SYMBOLIC_LINK_ALL_ACCESS, attributes, lpTarget);
      if (result != 0)
        log("\x1B[91mNative symlink creation failed: ${getMessageNt(result)}",
            level: 1000);
      else
        return lpHandle.value;
    } finally {
