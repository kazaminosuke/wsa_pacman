...
enum Architecture { amd64, i386, aarch64, arm, ppc64, ppc }

extension Architectures on Architecture {
  static final fullRegex = '(${[
  ...
  List<String> get labels => () {
        switch (this) {
          case Architecture.i386:
            return ["i386", "i686", "i586", "i486", "x86"];
...
class XapkReader extends IsolateRunner<String, APK_READER_FLAGS> {
  ...
  static final Directory _xapkTempDir =
  ...
  static Future<List<ProcessResult>> copyApkResources(
          List<ManifestXapk_ApkExpansion> expansions,
          String workingDir,
          String ipAddress,
          int port) =>
      Future.wait(() sync* {
        ...
        for (ManifestXapk_ApkExpansion exp in expansions) {
          ...
          final resourceDir =
              '${exp.installPath.startsWith('/') ? '' : '/sdcard/'}${path.dirname(exp.installPath)}';
          yield ADBUtils.pushToAddress(
                  ipAddress, port, exp.file, '/sdcard/$tempName',
                  workDir: workingDir)
              .timeout(const Duration(seconds: 30))
              .then((_) => ADBUtils.shellToAddress(ipAddress, port,
                      'mkdir -p "$resourceDir"; cd "$resourceDir"; mv /sdcard/$tempName ./$resourceName')
                  .timeout(const Duration(seconds: 30)));
        }
      }());

  static void installXApk(
      ...
      int timeout,
      FileDisposeQueue disposeLock,
      [bool downgrade = false]) async {
    if (apkFiles.isNotEmpty)
      log("INSTALLING \"${apkFiles.first}\" on on $ipAddress:$port...");
    disposeLock.clear();
    ...
    if (!isBootCompleted) {
      ...
      GState.errorDesc
          .update((_) => "WSAの起動完了を2分間待ちましたが、応答がありません。WSAを再起動してください。");
      return;
    }

    // 完全に起動したのを確認してから、本来のインストール（複数APK一括転送）コマンドを発行
    var installation = ADBUtils.installMultipleToAddress(
        ipAddress, port, apkFiles,
        downgrade: downgrade, workDir: workingDir);
  ...
  static List<String> _getApkList(ManifestXapk manifest) {
    ...
    if (manifest.splitApks.isNotEmpty) {
      bool isBaseApk(ManifestXapk_ApkFile fileInfo) =>
          fileInfo.id == 'base' || fileInfo.file == defaultBaseName;
      ManifestXapk_ApkFile? baseApk =
          manifest.splitApks.firstWhereOrNull(isBaseApk);
      if (manifest.splitApks.first == baseApk || baseApk == null)
        apkList = manifest.splitApks.map((e) => e.file);
      else
        apkList = [
          baseApk.file
        ].followedBy(manifest.splitApks.whereNot(isBaseApk).map((e) => e.file));
    } else if (manifest.splitConfigs.isNotEmpty) {
      Iterable<String> configFiles = manifest.splitConfigs.map((e) => '$e.apk');
      apkList = manifest.splitConfigs.contains(manifest.packageName)
          ? configFiles
          : [defaultBaseName].followedBy(configFiles);
    } else
      apkList = [defaultBaseName];

    final List<String> archApkList =
        apkList.where((file) => archRegex.hasMatch(file)).toList();
    ...
    for (final arch in Architecture.values) {
      ...
      for (final file in archApkList)
        if (regex.hasMatch(file)) return apkList.followedBy([file]).toList();
    }
    return apkList.followedBy([apkList.first]).toList();
  }

  void updateManifest(
      ManifestXapk manifest, Set<AndroidPermission> permissions) {
    executeInUi(() {
  ...
  void updateInstallInfo(ManifestXapk manifest, String installDir,
      List<String> apkList, FileDisposeQueue disposeLock) {
    executeInUi(() {
      if (manifest.packageName.isNotEmpty)
        ApkReader.loadInstallType(manifest.packageName, manifest.versionCode);
      GState.installCallback.$ =
          (ipAddress, port, lang, timeout, [downgrade = false]) => installXApk(
              installDir,
              apkList,
              manifest.expansions,
              ipAddress,
  ...
  void run() async {
    try {
      ...
      if (manifestFile == null) {
        ...
        final apkFiles = archive.files
            .where((f) => f.name.toLowerCase().endsWith('.apk'))
            .toList();
        if (apkFiles.isEmpty) throw Exception("No APK files found in archive");

        final baseApk = apkFiles
                .firstWhereOrNull((f) => f.name.toLowerCase() == 'base.apk') ??
        ...
        try {
          final result = Process.runSync('${Env.TOOLS_DIR}\\aapt.exe',
              ['dump', 'badging', extractedBaseApkPath]);
          if (result.exitCode == 0) {
            final out = result.stdout.toString();
            parsedPackage = RegExp(r"package:\s*name='([^']+)'")
                    .firstMatch(out)
                    ?.group(1) ??
                "";
            // ★ versionCodeをaaptから取得！
            parsedVersionCode = int.tryParse(RegExp(r"versionCode='([^']+)'")
                        .firstMatch(out)
                        ?.group(1) ??
                    "0") ??
                0;
            parsedVersion =
                RegExp(r"versionName='([^']+)'").firstMatch(out)?.group(1) ??
                    "1.0";

            final appMatch =
                RegExp(r"application:.*?label='([^']+)'.*?icon='([^']+)'")
                    .firstMatch(out);
            String? iconPath;
            if (appMatch != null) {
              ...
              parsedTitle = RegExp(r"application-label:'([^']+)'")
                      .firstMatch(out)
                      ?.group(1) ??
                  parsedTitle;
            ...
            // ★ アイコン取得の超強化！
            var iconFile =
                archive.findFile('icon.png'); // apkm直下に親切に置いてある場合はそれを使う
            if (iconFile == null && iconPath != null) {
              if (iconPath.endsWith('.png') || iconPath.endsWith('.webp')) {
                ...
                iconFile = archive.files.firstWhereOrNull((f) =>
                    f.name.contains(baseName) && f.name.endsWith('.png'));
              }
            }
            if (iconFile != null) {
              // ★ Uint8List.fromList(...) で包んで型を変換する！
              parsedIcon = Image.memory(
                  Uint8List.fromList(iconFile.content as List<int>));
            }

            final permMatches =
                RegExp(r"uses-permission:\s*name='([^']+)'").allMatches(out);
        ...
        if (parsedIcon != null) {
          ...
          ApkReader.setDefaultIcon(
              await waitFlag(APK_READER_FLAGS.LEGACY_ICON));
        }
        ...
        executeInUi(() {
          if (parsedPackage.isNotEmpty)
            ApkReader.loadInstallType(
                parsedPackage, parsedVersionCode); // ★ ここも修正
          GState.installCallback.$ = (ipAddress, port, lang, timeout,
                  [downgrade = false]) =>
              installXApk(installDir, apkList, [], ipAddress, port, lang,
                  timeout, disposeLock, downgrade);
          GState.apkInstallType.$ = InstallType.INSTALL;
        });
