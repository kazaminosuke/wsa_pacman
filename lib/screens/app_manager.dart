...
class _ScreenAppManagerState extends State<ScreenAppManager> {
  ...
  void _queueInfoBar(InfoBar Function(BuildContext context, void Function() close) builder) async {
    // 前の通知が出ている間は待機
  ...
  Future<void> _startScan(AppLocalizations lang) async {
    ...
    try {
      final adbProcess = await Process.run('adb', ['shell', 'pm', 'list', 'packages', '-3']);
  ...
  Future<bool> _backupRegistry(AppLocalizations lang, {bool isAuto = false}) async {
    String backupPath = '';
  ...
  Future<void> _executeCleanup(AppLocalizations lang) async {
    ...
    for (String package in _selectedPackages) {
      final appData = _detectedApps.firstWhere((app) => app['package'] == package, orElse: () => {});
  ...
  Widget _buildAppIcon(String? iconPath, String type) {
    ...
    if (iconPath != null && iconPath.isNotEmpty) {
      ...
      if (iconFile.existsSync()) {
        ...
        if (lowerPath.endsWith('.png') || lowerPath.endsWith('.jpg') || lowerPath.endsWith('.bmp') || lowerPath.endsWith('.webp') || lowerPath.endsWith('.ico')) {
          return ClipRRect(
          ...
          Card(
            child: Row(
              children: [
                ...
                Button(
                  onPressed: _selectedPackages.isEmpty ? null : () => _executeCleanup(lang),
                        ...
                        child: Row(
                          children: [
                            SizedBox(
                              ...
                              height: 32,
                              child: Center(
                                child: _buildAppIcon(app['displayIcon']?.toString(), app['type']),
                              ),
                            ),
                            ...
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(app['name'], style: theme.typography.bodyLarge),
                                  Text(packageId, style: theme.typography.caption),
                                ],
                              ),
                            ),
                            Checkbox(
